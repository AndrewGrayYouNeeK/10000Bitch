...
import { getSkin, getPipStyle, getBadge } from "@/lib/shopCatalog";
...
export function useCosmetics() {
  ...
  const buyItem = (type, item) => {
    ...
    const key = type === "skin" ? "owned_skins" : type === "pips" ? "owned_pips" : "owned_badges";
  ...
  const equipItem = (type, itemId) => {
    const key = type === "skin" ? "equipped_skin" : type === "pips" ? "equipped_pips" : "equipped_badge";
  ...
  return {
    ...
    equippedSkin, equippedPips, equippedBadge,
    addCoins,
    buyItem,
    equipItem,
  };
